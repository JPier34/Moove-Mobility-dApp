...
export default function Auction10Debugger() {
  ...
  const checkAuction10 = async () => {
    ...
    try {
      ...
      console.log("🔍 Fetching auction #10 data...");
      const data = await auctionContract.getAuction(10);
      ...
      console.log("📊 Auction #10 data:", processedData);

      // Check bids if it's a sealed bid auction
      if (processedData.auctionType === 2) {
        console.log("🔒 Fetching sealed bid data...");
        try {
          const bids = await auctionContract.getAuctionBids(10);
          console.log("📋 Raw bids data:", bids);

          const processedBids = bids.map((bid: any, index: number) => ({
            index,
            bidder: bid.bidder,
            bidHash: bid.bidHash,
            amount: bid.amount ? ethers.formatEther(bid.amount) : "0",
            isRevealed: bid.isRevealed || false,
            isUserBid: bid.bidder.toLowerCase() === address.toLowerCase(),
          }));

          setBidsData(processedBids);
          console.log("📋 Processed bids:", processedBids);
        } catch (bidError) {
          console.warn("⚠️ Could not fetch bids:", bidError);
          setBidsData([]);
        }
      }
      ...
      console.error("❌ Error fetching auction #10:", err);
  ...
  const checkNotificationLogic = (data: any) => {
    ...
    const logic = {
      ...
      timeSinceExpired: data.isExpired
        ...
        : 0,
      // Sealed bid specific logic
      shouldHaveSealedBidNotification:
        data.auctionType === 2 && (data.status === 2 || data.status === 3),
    };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
      ...
      <div className="space-y-4">
        <div className="flex gap-2">
          <button
            onClick={checkAuction10}
            disabled={loading}
            className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50"
          >
            {loading ? "Checking..." : "Check Auction #10"}
          </button>
        ...
        {auctionData && (
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            ...
            {/* Sealed Bid Bids Information */}
            {auctionData.auctionType === 2 && bidsData.length > 0 && (
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded">
                ...
                <div className="text-sm text-blue-700 dark:text-blue-300">
                  <div className="mb-2">
                    <strong>Total Bids:</strong> {bidsData.length}
                  </div>
                  <div className="mb-2">
                    <strong>Your Bids:</strong>{" "}
                    {bidsData.filter((bid) => bid.isUserBid).length}
                  </div>
                  {bidsData.map((bid, index) => (
                    <div key={index} className="ml-4 mb-1">
                      <strong>Bid {index + 1}:</strong>{" "}
                      {bid.isUserBid ? "👤 YOUR BID" : "👥 Other"} -{" "}
                      {bid.isRevealed ? "✅ Revealed" : "🔒 Sealed"} -{" "}
                      {bid.amount} ETH
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded">
              ...
              <div className="text-sm text-blue-700 dark:text-blue-300">
                ...
                {auctionData.auctionType === 2 &&
                  (auctionData.status === 2 || auctionData.status === 3) && (
                    <div className="text-purple-600 dark:text-purple-400">
                      🔒 Should have SEALED_BID notification
                    </div>
                  )}
                ...
                {!auctionData.isExpired && (
                  <div className="text-blue-600 dark:text-blue-400">
                    ⏰ Auction is still ACTIVE - no notifications yet
                  </div>
                )}
                {auctionData.isExpired &&
                  auctionData.status === 1 &&
                  !auctionData.isUserWinner && (
                    <div className="text-gray-600 dark:text-gray-400">
                      ℹ️ Auction expired but you are not the winner
                    </div>
                  )}
              </div>
            </div>
