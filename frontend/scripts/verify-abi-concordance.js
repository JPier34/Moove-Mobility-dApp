 ...
 * dal contratto deployato sulla blockchain
 */
...
// Ottieni ABI dal contratto deployato
async function getDeployedABI(contractAddress, provider) {
  try {
    // Prova a ottenere il codice del contratto
    const code = await provider.getCode(contractAddress);
    if (code === "0x") {
      throw new Error("Contratto non trovato all'indirizzo specificato");
    }

    // Per ora, restituiamo null perché non possiamo facilmente estrarre l'ABI dal bytecode
    // In futuro potremmo usare strumenti come etherscan API o altri servizi
    return null;
  } catch (error) {
    console.error(
      `❌ Errore nel recupero ABI deployato per ${contractAddress}:`,
      error.message
    );
    return null;
  }
...
// Funzione principale
async function main() {
  console.log("🔍 Verifica concordanza ABI - Moove dApp\n");
  ...
  for (const contractName of contracts) {
    ...
    console.log(`📍 Indirizzo: ${CONTRACT_ADDRESSES[contractName]}`);
    ...
    // Verifica se il contratto esiste sulla blockchain
    try {
      const code = await provider.getCode(CONTRACT_ADDRESSES[contractName]);
      if (code === "0x") {
        console.log(`❌ Contratto non trovato sulla blockchain`);
      } else {
        console.log(
          `✅ Contratto trovato sulla blockchain (${code.length} bytes)`
        );
      }
    } catch (error) {
      console.log(`❌ Errore nel verificare contratto: ${error.message}`);
    }
  }
